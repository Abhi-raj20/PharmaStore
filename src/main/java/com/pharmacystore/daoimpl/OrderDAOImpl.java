...
public class OrderDAOImpl implements OrderDao {
	...
	public boolean cancelOrder(Order order) {
		...
		try(Connection con = DbConnection.getDatabaseConnection()){
			PreparedStatement pst = con.prepareStatement("update productorder set cancelled = true where orderid = ?");
	...
	public int[] getProductDetailsFromOrderId(Order order) {
		...
		try(Connection con = DbConnection.getDatabaseConnection()){
			...
			PreparedStatement pst = con.prepareStatement("select * from productorder where oderid = ?");
			...
			if(rs.isBeforeFirst())
			{
				while(rs.next())
				{
					...
					product[1] = rs.getInt("orderdunits");
